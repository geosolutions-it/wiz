...
class WaterRequests extends CActiveRecord
{
	...
	/**
	 * Converts timestamp into a string human readable (only year part)
	 */
	...
	/**
	 * Converts timestamp into a string human readable (only month part)
	 */
	...
	public function imageSRC() {
		...
		$bbox = $this->getBBox();
		// adjust image size
		$ratio = ($bbox['xmax']-$bbox['xmin']+2*$buffer)/($bbox['ymax']-$bbox['ymin']+2*$buffer);  // ratio x/y = width/height
		$max_height= 600;
		...
		$resolution=($bbox['xmax']-$bbox['xmin']+2*$buffer)/$width;
		//Yii::log($resolution, CLogger::LEVEL_INFO, "imageSRC_wr");  // DEBUG
		$sfondo = 'idigm50kgrey';
		if(0.1 <= $resolution && $resolution < 0.28)   // from 1 to 1k
			$sfondo = Yii::app()->params['geoserver']['pdf_wms']['1000']; //'rst2k_liv3';
		if(0.28 <= $resolution && $resolution < 3.08)  // from 1k to 11k
			$sfondo = Yii::app()->params['geoserver']['pdf_wms']['11000']; //'idrst10k';
		if(3.08 <= $resolution && $resolution < 16.80) // from 11k to 60k
			$sfondo = Yii::app()->params['geoserver']['pdf_wms']['60000']; //'idigm50kgrey';
		if(16.80 <= $resolution && $resolution < 66)   // from 60k to 200k
			$sfondo = Yii::app()->params['geoserver']['pdf_wms']['200000']; //'rst100k';
		...
		$ret = Yii::app()->params['geoserver']['protocol'].
				...
				'&request='.$request.
				'&layers='.Yii::app()->params['geoserver']['workspace'].':'.$sfondo.
						 	','.
