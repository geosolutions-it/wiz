...
class WaterFaultsController extends Controller
{
	/**
	...
	{
		if (!Yii::app()->user->checkAccess('viewFault'))
			throw new CHttpException(403,Yii::t('http_status', '403'));
		
		$model=new WaterFaults('search');
		$model->unsetAttributes();  // clear any default values
	    if(isset($_GET['WaterFaults']))
	        $model->attributes =$_GET['WaterFaults'];
	...
	{
		if (!Yii::app()->user->checkAccess('createFault'))
			throw new CHttpException(403,Yii::t('http_status', '403'));
	
		$model=new WaterFaults;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		$model->setScenario('new_water_qualities');
		if(isset($_POST['WaterFaults']))
		{
			$model->attributes=$_POST['WaterFaults'];
			if($_POST['WaterFaults']['color']{0} != '#') 
				$model->setAttribute('color','#'.$_POST['WaterFaults']['color']);
	...
	{
		if (!Yii::app()->user->checkAccess('updateFault'))
			throw new CHttpException(403,Yii::t('http_status', '403'));
	...
	{
		$model=WaterFaults::model()->findByPk($id);
		if($model===null)
