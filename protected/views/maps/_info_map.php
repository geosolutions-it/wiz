...
<?php if(isset($layerslist['service_areas'])) : ?>			 			
			    ...
			    eventListeners: {
			        getfeatureinfo: function(event) {
						if(event.text!='')
						{
							...
							var url = <?php echo CJSON::encode(CController::createUrl('serviceArea/query',array('id_area'=>'-id_a'))); ?>;
							var _url = url.replace('-id_a', event.text);
							 $.ajax({
								url: _url,
								cache: false,
								dataType: 'html',
	    	...
	    	panel.addControls([sa_margin_tool]);
	    	
			// *** FINE service_area_margin_tool *** 			
<?php endif ?>
        }
